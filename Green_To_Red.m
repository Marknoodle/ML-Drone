...
%%%% This file's purpose is to measure px distance so we can convert it to flight distance
...
pixelDistance = pdist([XredCentre,YredCenter;XgreenCentre,YgreenCenter] ,'euclidean'); %1 meter is about 295 pixels

flyDistance = pixelDistance * (1/295); %calculate actual flight distance
[Angle,Radian]=getAngle([XgreenCentre,YgreenCenter],[XredCentre,YredCenter]);

% Now we need to determine where the drone is and where it's facing w/
% respect to the location of the red paper.

trueAngle = deg2rad(abs(Angle - 180)); %This is only valid for when the drone is below and to the left of the red page
...
if trueAngle > 5 % we do not want to turn if the angle is really really small
turn(r,trueAngle);
end
